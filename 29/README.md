...
```bash
iface: eth1 #указываем интерфейс
pxe_ip: 192.168.1.11
...
```
#### Создаем плейбук и inventory файл, запускаем
##### install_pxe.yml
...
```bash
[pxe]
192.168.1.11 ansible_user=hwuser
```
```bash
ansible-playbook -i inventory.ini install_pxe.yml
```
